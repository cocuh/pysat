...
class Solver(object):
    ...
    Solver object
    Attributes:
        litlist(LitList): literal list object
        learnt_clause(list): learnt clauses
        
        level(int): current level in the solver
        root_level(int)
        
        propagate_history(dict): implication graph
            key(int) -> level
            value(list) -> BindLit  
        decide_history(dict): 
            key(int) -> level
        ...
        pickup_type(str):
            "random" -> pick up randomly
            "order" -> pick up in order 
        output_type(str):
            "json" -> output json style
            "default" -> output default log style
    """
    def __init__(self):
    ...
    def print_result(self):
        ...
        if not self.output_type == 'default':
            return
    ...
    def __str__(self):
        if self.output_type == 'default':
            ...
            blitlize = lambda x:x.lit.id if x.get_raw_sign() else -x.lit.id
            res = OrderedDict()
            res['status'] = self.status
            res['lit'] = OrderedDict()
            res['clause'] = OrderedDict()
    ...
    def output(self, level='info'):
        if self.output_type == 'default':
            ...
            print(self)
        if self.sleep_time:
            time.sleep(self.sleep_time*1.0/1000)
        sys.stdout.flush()
...
if __name__ == '__main__':
    ...
    solver.output_type = arguments.output_type
    solver.sleep_time = arguments.sleep
    
    if (arguments.debug and solver.output_type == 'default'):
